      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <header className="mb-16">
          <div className="flex items-center gap-3 text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-[0.2em] mb-6">
            <Link to="/profile" className="hover:text-[var(--color-accent)] transition-colors">Profile</Link>
            <ChevronRight size={12} className="opacity-30" />
            <span className="text-[var(--color-primary-dark)]">Order History</span>
          </div>
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div className="max-w-2xl">
              <h1 className="text-4xl sm:text-6xl font-['Fraunces'] font-semibold text-[var(--color-primary-dark)] tracking-tight">My Orders</h1>
              <p className="mt-4 text-xl text-[var(--color-text-muted)] font-medium italic leading-relaxed">
                "View all your orders and track their delivery status."
              </p>
            </div>
            ...
            <div className="flex flex-col items-center lg:items-end bg-white/50 backdrop-blur-md p-6 px-10 rounded-[2.5rem] border border-white shadow-xl">
              <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--color-text-muted)] mb-2">Total Orders</div>
              <div className="text-4xl font-['Fraunces'] font-bold text-[var(--color-accent)]">{orders.length} Orders</div>
            </div>
          </div>
        </header>
        ...
        {isLoading ? (
          ...
          <div className="py-24 text-center bg-[var(--color-sand)]/10 border-2 border-dashed border-[var(--color-stone)]/10 rounded-[2rem] sm:rounded-[4rem]">
            ...
            <p className="mt-2 text-[var(--color-text-muted)] italic">"You don't have any orders in this category."</p>
          </div>
        ) : (
...
function OrderItem({ order }) {
  return (
    <SurfaceCard className="bg-white border-[var(--color-stone)]/5 p-0 overflow-hidden shadow-xl rounded-[2rem] sm:rounded-[3.5rem] group transition-all hover:shadow-2xl hover:border-[var(--color-accent)]/10">
        ...
        <div className="flex-1 p-8 sm:p-12">
          <div className="flex flex-wrap items-center justify-between gap-6 mb-10">
            ...
            <div className="text-right">
              <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-muted)] mb-1">Order Total</p>
            ...
            {(order.items ?? []).map((item, i) => (
              <div key={`${order.id}-${i}`} className="flex items-center gap-5 p-5 rounded-[2rem] bg-[var(--color-sand)]/10 border border-transparent hover:border-[var(--color-accent)]/10 transition-all">
                ...
                <div className="min-w-0">
                  ...
                  {item.variantLabel && (
                    <p className="text-[10px] font-black text-[var(--color-accent)] uppercase tracking-tighter mb-1 italic">
         ...
         {/* Actions */}
        <div className="w-full xl:w-[400px] bg-[var(--color-sand)]/10 p-8 sm:p-12 border-l border-[var(--color-stone)]/5 flex flex-col justify-between">
          <div className="space-y-8">
            <div>
              <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--color-text-muted)] mb-4">Seller</h3>
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="font-bold text-[var(--color-primary-dark)] truncate">{order.shopName || `Store #${order.sellerId}`}</p>
                  <p className="text-[10px] font-bold text-[var(--color-accent)] uppercase tracking-widest mt-1">Verified Seller</p>
                </div>
              </div>
            </div>

            <div>
              <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--color-text-muted)] mb-4">Actions</h3>
          ...
          <div className="mt-10 flex items-center justify-between text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest py-4 border-t border-[var(--color-stone)]/10">
            ...
            <div className="flex items-center gap-2 text-[var(--color-accent)]">
              Order Verified <CheckCircle2 size={12} />
            </div>
          </div>
        </div>
      </div>
